    ...
    <>
      ...
      <section className="bg-gradient-to-br from-brand-600 to-brand-900 text-white">
        <div className="mx-auto max-w-6xl px-4 py-20 sm:py-28 text-center">
          ...
          <h1 className="mt-5 text-4xl sm:text-5xl font-bold tracking-tight max-w-3xl mx-auto">
            Win more jobs. Do less admin.
          </h1>
          <p className="mt-5 text-lg text-brand-100 max-w-2xl mx-auto">
            LeadFlow Pro is the all-in-one platform for tradies — capture leads, send quotes,
            schedule jobs, invoice and get paid. The follow-ups run themselves.
          </p>
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/register" className="rounded-lg bg-white px-6 py-3 font-semibold text-brand-700 hover:bg-brand-50">
              Start your 14-day free trial
            </Link>
            <Link href="/pricing" className="rounded-lg border border-white/30 px-6 py-3 font-semibold text-white hover:bg-white/10">
              See pricing
            </Link>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-20">
        <div className="grid gap-4 sm:grid-cols-3 max-w-3xl mx-auto text-center">
          {["Built for AU & NZ (GST, Xero)", "Works on the tools — mobile-ready", "Your data, your branding"].map((t) => (
            <div key={t} className="flex items-center justify-center gap-2 text-sm font-medium text-gray-700">
              <CheckCircle2 className="w-5 h-5 text-green-600 flex-shrink-0" /> {t}
            </div>
          ))}
        </div>
      </section>
